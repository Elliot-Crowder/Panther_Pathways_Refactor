...
document.addEventListener("DOMContentLoaded", () => {
	fetch("vector_map.svg")
		...
		.then(async (svgContent) => {
			...
			// Send the node circles to the back so they paint beneath the map artwork
			// and stay invisible. Pathfinding/route drawing read their cx/cy via
			// getElementById, which is independent of paint order, so behavior is
			// unchanged. Moving the group element also carries along any circles
			// loadNodes appends into it asynchronously.
			const main = svg.querySelector("#main");
			main.prepend(svg.querySelector("#nodes"));
